/**
 * Forked from https://github.com/edlsh/pi-ask-user at v0.15.0 (commit 705fdc6), MIT License
 * (see ./LICENSE). Local changes are marked "pi-swarm:" in comments; the upstream tests are
 * bun:test and are not carried here (see .pi/test/tools/ask-user-layout.test.ts).
 */
/**
...
export default function(pi: ExtensionAPI) {
   pi.registerTool({
      ...
      promptSnippet:
         "Ask the user one focused question with optional multiple-choice answers. Use question (not questions) for normal use.",
      promptGuidelines: [
      ...
      parameters: Type.Object({
         ...
         header: Type.Optional(Type.String({ description: "Short dialog heading" })),
         kind: Type.Optional(Type.String({ description: "Compatibility alias for question type" })),
         questions: Type.Optional(Type.Array(Type.Object({
            prompt: Type.Optional(Type.String({ description: "Question text" })),
            question: Type.Optional(Type.String({ description: "Alias for prompt" })),
         }), { description: "Optional questionnaire fallback" })),
         choices: Type.Optional(Type.Array(Type.String({ description: "Choice label" }))),
      ...
      async execute(_toolCallId, params, signal, onUpdate, ctx) {
         ...
         const {
            ...
            context,
            // Accept the common questionnaire-shaped fallback as a single
            // prompt instead of rejecting it before the UI can help.
            questions,
            choices,
            options: rawOptions = [],
            allowMultiple = false,
         ...
         if (!question) {
            return { content: [{ type: "text", text: "A non-empty question is required." }], isError: true, details: { error: "question is required" } };
         }
         ...
         const options = effectiveOptions.map(coerceOption).filter((option): option is QuestionOption => option !== null);
         ...
         if (effectiveOptions.length > 0 && options.length === 0) {
            return {
               content: [
                  {
                     type: "text",
                     text:
                        `All ${effectiveOptions.length} option(s) were malformed, so nothing could be shown to the user. `
