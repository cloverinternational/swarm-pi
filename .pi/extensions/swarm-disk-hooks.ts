...
export function registerDiskHooks(pi: any, options: { cwd?: string; home?: string; defaultTimeoutMs?: number } = {}) {
  ...
  for(const h of loadFiles(cwd,home)) runtime.register({name:h.name,priority:h.priority,events:h.events,phase:h.events.some((e:string)=>e.includes("after"))?"after":"before",timeoutMs:h.timeoutMs,handle:async event=>{const controller=new AbortController();const child=await import("node:child_process");const p=child.exec(h.command,{cwd,timeout:h.timeoutMs,maxBuffer:1024*1024},(error:any,_stdout:any,stderr:any)=>{if(error)throw error;if(h.action==="block_on_output"&&stderr)return;}); await new Promise((resolve,reject)=>{p.on("exit",(code:number)=>{if(h.action==="block_exit2"&&code===2)reject(new Error("disk hook blocked (exit 2)"));else if(h.action==="block"&&code!==0)reject(new Error(`disk hook failed (${code})`));else resolve(undefined)});p.on("error",reject)}); void controller;}});
