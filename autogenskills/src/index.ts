...
export function registerAutoSkills(pi: any, config: Config = {}) {
  ...
  const updateBudgetWidget = (ctx: any) => ctx?.ui?.setWidget?.("swarm-autogen-budget", manager.budgetWidgetLines(), { placement: "belowEditor" });
  register("tool_call", (e: any, ctx: any) => { updateBudgetWidget(ctx); return manager.gateTool(e.toolName ?? e.tool_name, e.input ?? e.params); });
  ...
  register("tool_result", (e: any, ctx: any) => { manager.observeTool(!e.isError && !e.error && !e.result?.isError, e.toolName ?? e.tool_name, e.input ?? e.params); updateBudgetWidget(ctx); });
  register("turn_end", (_e: any, ctx: any) => { const nudge = manager.observeTurn(); updateBudgetWidget(ctx); if (nudge) ctx.ui?.notify(nudge, "info"); });
  register("session_start", (_e: any, ctx: any) => { manager.rehydrate(ctx.sessionManager?.getEntries?.() ?? []); updateBudgetWidget(ctx); });
