...
export function validateCron(expression) {
    ...
    if (!normalized)
        throw new Error("cron parameter is required");
...
export function nextCronTime(expression, after) {
...
export function humanReadableCron(expression) {
...
export function parseDelay(value, maxMs = 365 * 24 * 60 * 60 * 1_000) {
    const input = value.trim();
    if (!input)
        throw new Error("delay parameter is required");
    let total = 0;
    let offset = 0;
    ...
    if (rounded > maxMs)
        throw new Error(`delay exceeds maximum of ${maxMs}ms`);
    return rounded;
}
