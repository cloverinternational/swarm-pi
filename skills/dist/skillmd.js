...
const isPlain = (s) => s.type === undefined || s.type === Scalar.PLAIN;
...
function scalarText(node) {
    if (!isScalar(node))
        throw new DecodeError("cannot unmarshal non-scalar into string");
    const text = String(node.value ?? "");
    if (isPlain(node) && NULLS.has(text))
        return null; // !!null → zero value, no error
    return text;
}
function decodeString(node) { return scalarText(node) ?? ""; }
function decodeBool(node) {
    if (!isScalar(node))
        throw new DecodeError("cannot unmarshal non-scalar into bool");
    const text = String(node.value ?? "");
    if (isPlain(node)) {
        if (NULLS.has(text))
            return false;
    ...
    if (BOOL11_TRUE.has(text))
        return true;
    if (BOOL11_FALSE.has(text))
        return false;
    throw new DecodeError(`cannot unmarshal ${JSON.stringify(text)} into bool`);
}
function decodeInt(node) {
    if (!isScalar(node))
        throw new DecodeError("cannot unmarshal non-scalar into int");
    const text = String(node.value ?? "");
    if (!isPlain(node))
        throw new DecodeError("cannot unmarshal !!str into int");
    if (NULLS.has(text))
        return 0;
    ...
    if (HEX_RE.test(text))
        return Number.parseInt(clean.replace(/0x/i, ""), 16) * (clean.startsWith("-") ? -1 : 1);
    if (BIN_RE.test(text))
        return Number.parseInt(clean.replace(/0b/i, ""), 2) * (clean.startsWith("-") ? -1 : 1);
    if (OCT_RE.test(text))
        return Number.parseInt(clean.replace(/^([-+]?)0o?/, "$1"), 8);
    throw new DecodeError(`cannot unmarshal ${JSON.stringify(text)} into int`);
}
function decodeStrings(node) {
    if (isScalar(node) && isPlain(node) && NULLS.has(String(node.value ?? "")))
        return [];
    if (!isSeq(node))
        throw new DecodeError("cannot unmarshal non-sequence into []string");
    return node.items.map(item => decodeString(item));
}
/** yaml.Unmarshal(frontmatter, &SkillMetadata) — throws DecodeError like yaml.v3. */
export function decodeMetadataYAML(text) {
    const doc = parseDocument(text, { schema: "failsafe", uniqueKeys: true, strict: true });
    if (doc.errors.length)
        throw new DecodeError(doc.errors[0].message);
    const meta = emptyMetadata();
    const root = doc.contents;
    if (root === null || root === undefined)
        return meta;
    if (isScalar(root) && isPlain(root) && NULLS.has(String(root.value ?? "")))
        return meta;
    if (!isMap(root))
        throw new DecodeError("cannot unmarshal non-mapping into SkillMetadata");
    for (const pair of root.items) {
        const key = isScalar(pair.key) ? String(pair.key.value ?? "") : "";
        const spec = FIELDS[key];
        if (!spec)
            continue; // unknown keys are ignored (no KnownFields)
        const [field, kind] = spec;
        const value = pair.value;
        if (kind === "string")
            meta[field] = decodeString(value);
        else if (kind === "bool")
            meta[field] = decodeBool(value);
        else if (kind === "int")
            meta[field] = decodeInt(value);
        else
            meta[field] = decodeStrings(value);
    }
    return meta;
}
/** parser.go sanitizeFrontmatterYAML. */
export function sanitizeFrontmatterYAML(lines) {
    let changed = false;
    ...
    lines.forEach((raw, i) => {
        ...
        if ("\"'[{|>&*!#".includes(val[0]))
            return;
        if (!(val.includes(": ") || val.endsWith(":") || val.includes(" #") || val.includes("\t")))
            return;
...
export function unquoteFrontmatterScalar(s) {
...
export function parseFrontmatterLenient(lines) {
        ...
        switch (key) {
            ...
            case "tags": {
                ...
                for (const t of trimmed.split(",")) {
                    const tag = unquoteFrontmatterScalar(t.trim());
                    if (tag !== "")
                        md.tags.push(tag);
                }
                break;
...
export function extractDescriptionFromMarkdown(content) {
    ...
    for (const line of content.split("\n")) {
        ...
        if (!skippedHeading && trimmed.startsWith("# ")) {
            skippedHeading = true;
            continue;
        }
    ...
    if (byteLength(desc) > 256)
        desc = Buffer.from(desc, "utf8").subarray(0, 253).toString("utf8").replace(/\uFFFD+$/, "") + "\u2026";
    return desc;
}
/** parser.go ParseSkillMDContent (hooks are not modelled; they never reach the wire). */
export function parseSkillMDContent(data) {
    ...
    const frontmatter = [], contentLines = [];
    ...
    if (frontmatter.length) {
        try {
            metadata = decodeMetadataYAML(frontmatter.join("\n"));
        }
        catch (error) {
            let recovered = false;
            ...
            if (changed) {
                try {
                    metadata = decodeMetadataYAML(sanitized);
                    recovered = true;
                }
                catch { /* fall through */ }
            }
    ...
    if (metadata.description === "" && instructions !== "")
        metadata.description = extractDescriptionFromMarkdown(instructions);
...
export function validateName(name) {
    ...
    if (byteLength(name) > MAX_NAME_LENGTH)
        return `name exceeds maximum length of ${MAX_NAME_LENGTH} characters (got ${byteLength(name)})`;
...
export function validateDescription(desc) {
    ...
    if (byteLength(desc) > MAX_DESCRIPTION_LENGTH)
        return `description exceeds maximum length of ${MAX_DESCRIPTION_LENGTH} characters (got ${byteLength(desc)})`;
    return undefined;
}
/** LoadSkillWithValidation's fatal gate: the first fatal validation error, if any. */
export function fatalValidationError(metadata) {
    const name = validateName(metadata.name);
    if (name)
        return `validation error: [error] name: ${name}`;
    const description = validateDescription(metadata.description);
    if (description)
        return `validation error: [error] description: ${description}`;
    return undefined;
}
