...
func TestHTTPClientDefaultTimeout(t *testing.T) {
	for _, transportMode := range []string{"", "resilient", "default"} {
		t.Run(transportMode, func(t *testing.T) {
			client := createResilientHTTPClientOpenAI(Config{TransportMode: transportMode})
...
func TestHTTPClientTimeoutOverrides(t *testing.T) {
	const configuredTimeout = 37 * time.Second

	client := createResilientHTTPClientOpenAI(Config{Timeout: int(configuredTimeout / time.Second)})
	if client.Timeout != configuredTimeout {
		t.Fatalf("configured Timeout = %s, want %s", client.Timeout, configuredTimeout)
	}

	client = createResilientHTTPClientOpenAI(Config{Timeout: -1})
	if client.Timeout != 0 {
