...
func buildScriptCommandString(shell, command string) string {
	return fmt.Sprintf("%s -c %s", shellQuote(shell), shellQuote(command))
}
...
// GetBufferingMethodName returns a human-readable name for the detected buffering method.
// Useful for debugging and logging.
func GetBufferingMethodName() string {
	method := detectBufferingMethod()
	switch method {
	case bufferMethodStdbuf:
		return "stdbuf (GNU coreutils)"
	case bufferMethodScriptBSD:
		return "script (BSD PTY)"
	case bufferMethodScriptLinux:
		return "script (util-linux PTY)"
	case bufferMethodDirect:
		return "direct (no buffering control)"
	default:
		return "unknown"
	}
}
